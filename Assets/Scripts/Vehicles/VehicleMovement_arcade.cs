...
public class VehicleMovement_arcade : MonoBehaviour
{
    ...
    void ResetWheels(Vector3 position)
    {
        ...
        Debug.Log(string.Format("ResetWheels {0}, {1}, {2}, Rot {3}", position.x, position.y, position.z, yaw));
    }
    ...
    void UpdateInput()
    {
        ...
        if (Input.GetKey(KeyCode.R) && controllable)
        {
            ...
            if (numHits > 0)
            {
                ...
                ResetWheels(resetPos);
            }
            else
            {
                // Hard reset
                ResetWheels(new Vector3(-69.48f, 5.25f, 132.71f));
            }
        }
